...
export function NeedsMap(props: NeedsMapProps) {
  ...
  return (
    <View>
      <PinMap {...props} style={styles.small} onPressMap={() => setExpanded(true)} />
      <Pressable
        accessibilityRole="button"
        accessibilityLabel="expand the map"
        hitSlop={8}
        onPress={() => setExpanded(true)}
        style={[styles.expand, { backgroundColor: theme.backgroundElement, boxShadow: `0 1px 3px ${theme.shadow}` }]}>
        <SymbolView
...
function FullScreenMap({ onClose, ...props }: NeedsMapProps & { onClose: () => void }) {
  ...
  return (
    <View style={[styles.fill, { backgroundColor: theme.background }]}>
      <PinMap {...props} style={styles.fill} onPressMap={() => props.onSelectOrganization(null)} />
      ...
      <Pressable
        ...
        style={[
          styles.close,
          { top: insets.top + Spacing.two, backgroundColor: theme.backgroundElement, boxShadow: `0 1px 3px ${theme.shadow}` },
        ]}>
      ...
      {selected ? (
        <View
          style={[
            styles.sheet,
            { paddingBottom: insets.bottom + Spacing.three, backgroundColor: theme.background, boxShadow: `0 -2px 8px ${theme.shadow}` },
          ]}>
...
function PinMap({
  ...
  onPressMap,
  style,
}: NeedsMapProps & { onPressMap: () => void; style: StyleProp<ViewStyle> }) {
  const theme = useTheme();
